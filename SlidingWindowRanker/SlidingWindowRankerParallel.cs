...
public partial class SlidingWindowRankerParallel<T> where T : IComparable<T>
{
    private readonly object _lock = new();
    private readonly List<Partition<T>> _partitions = [];

    private readonly SemaphoreSlim _semaphore = new(1, 1); // Initialize with a count of 1
    ...
    public SlidingWindowRankerParallel(List<T> initialValues, int partitionCount, int windowSize = -1)
    {
        if (windowSize < 0)
        {
    ...
    {
        _semaphore.Wait(); // Block so we don't re-enter while we are doing insert and remove and AdjustPartitionsLowerBounds
        _ = _semaphore.CurrentCount;
        ...
        // Kick off DoInsert and DoRemove in parallel
        var insertTask = Task.Run(DoInsert);
        var removeTask = Task.Run(DoRemove);

        // Wait for both tasks to complete, then kick off AdjustPartitionsLowerBounds
        Task.WhenAll(insertTask, removeTask).ContinueWith(_ =>
        {
            Task.Run(AdjustPartitionsLowerBounds);
        }).ContinueWith(_ =>
        {
            // Release the semaphore after all tasks have been started
            var c = _semaphore.CurrentCount;
            _semaphore.Release();
        });
